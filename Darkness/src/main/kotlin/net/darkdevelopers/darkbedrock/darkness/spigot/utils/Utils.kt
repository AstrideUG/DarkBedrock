...
object Utils {
    ...
    @Deprecated(
        "",
        ReplaceWith("TitleUtils(player)", "net.darkdevelopers.darkbedrock.darkness.spigot.utils.TitleUtils"),
    ...
    @Deprecated(
        "",
        ReplaceWith(
            "TitleUtils(player.sendTitle(title)",
            "net.darkdevelopers.darkbedrock.darkness.spigot.utils.Utils",
            "org.bukkit.entity.Player",
            "net.darkdevelopers.darkbedrock.darkness.spigot.utils.TitleUtils"
        ),
        DeprecationLevel.ERROR
    )
    fun sendTitle(player: Player?, title: String): Utils {
        sendPacket(
            player, PacketPlayOutTitle(
    ...
    @Deprecated(
        "",
        ReplaceWith(
            "TitleUtils(player).sendSubTitle(title)",
            "net.darkdevelopers.darkbedrock.darkness.spigot.utils.Utils",
            "org.bukkit.entity.Player",
            "net.darkdevelopers.darkbedrock.darkness.spigot.utils.TitleUtils"
        ),
    ...
    @Deprecated(
        "",
        ReplaceWith(
            "Utils.getTitleBuilder(player).sendTimings(title)",
            "net.darkdevelopers.darkbedrock.darkness.spigot.utils.Utils",
            "org.bukkit.entity.Player",
            "net.darkdevelopers.darkbedrock.darkness.spigot.utils.TitleUtils"
        ),
    ...
    fun sendAllPlayerParticle(player: Player, particleType: EnumParticle, loc: Location, speed: Float, amount: Int) =
        goThroughAllPlayers { sendPlayerParticle(player, particleType, loc, speed, amount) }
...
//            sendPacket(player, PacketPlayOutChat(IChatBaseComponent.ChatSerializer.a("{\"text\":\"$message\"}"), 2.toByte()))
    ...
    fun getTime(inputTime: Long): String {
        ...
        val day = time / 86400
        var remainingTime = ""
        if (day == 1L) remainingTime = "$remainingTime${IMPORTANT}ein$TEXT Tag " else if (day != 0L) remainingTime =
                "$remainingTime$IMPORTANT$day$TEXT Tage "
        if (hour == 1L) remainingTime =
                "$remainingTime${IMPORTANT}eine$TEXT Stunde " else if (hour != 0L) remainingTime =
                "$remainingTime$IMPORTANT$hour$TEXT Stunden "
        if (min == 1L) remainingTime = "$remainingTime${IMPORTANT}eine$TEXT Minute " else if (min != 0L) remainingTime =
                "$remainingTime$IMPORTANT$min$TEXT Minuten "
        if (sec == 1L) remainingTime = "$remainingTime$IMPORTANT$sec$TEXT Sekunde " else if (time != 0L) remainingTime =
                "$remainingTime$IMPORTANT$sec$TEXT Sekunden "
        return if (remainingTime.isBlank()) "${IMPORTANT}0 ${TEXT}Sekunden " else remainingTime.substring(
            0,
            remainingTime.length - 1
        )
    }
}