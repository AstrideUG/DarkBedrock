...
class MySQL(private val mySQLData: MySQLData) {
    ...
    constructor(config: Config = GsonConfig(ConfigData("configs", "mysql.json"))) : this(try {
    ...
    suspend fun preparedQuery(preparedQuery: PreparedStatement) = suspendCoroutine<ResultSet> { preparedQuerySync(preparedQuery) }
    ...
    private fun connect() = if (isOpen()) {
        ...
        connection = DriverManager.getConnection("jdbc:mysql://${mySQLData.host}:${mySQLData.port}/${mySQLData.database}", mySQLData.username, mySQLData.password)
