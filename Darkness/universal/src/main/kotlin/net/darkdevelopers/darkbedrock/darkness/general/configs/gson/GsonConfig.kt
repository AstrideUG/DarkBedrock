...
open class GsonConfig(
    override val configData: ConfigData, var jsonObject: JsonObject = JsonObject(),
    ...
    companion object {
        ...
        fun multiPlaceJsonObject(
            ...
            if (canBeNull) multiPlaceJsonObject(
                config.getAs<JsonElement>(name),
                name,
                directory
            )
            else multiPlaceJsonObject(
                config.getAsNotNull<JsonElement>(name),
                name,
                directory
            )
        ...
        fun multiPlaceJsonObject(
            ...
            is JsonPrimitive -> if (element.isString) GsonService.load(
                GsonConfig(ConfigData(
...
//			jsonObject = Gson().fromJson(String(Files.readAllBytes(getFile().toPath())), JsonObject::class.java)
    ...
    @Deprecated(
        ...
        ReplaceWith(
            "GsonService.save(configData, jsonObject)",
            "net.darkdevelopers.darkbedrock.darkness.general.configs.gson.GsonService"
        )
    )
    ...
    @Deprecated(
        "Use GsonService",
        ReplaceWith(
            "GsonService.formatJson(jsonObject)",
            "net.darkdevelopers.darkbedrock.darkness.general.configs.gson.GsonService"
        )
    )
    ...
    override fun toString(): String =
        "GsonConfig(configData=$configData, jsonObject=$jsonObject, serializeNulls=$serializeNulls)"
