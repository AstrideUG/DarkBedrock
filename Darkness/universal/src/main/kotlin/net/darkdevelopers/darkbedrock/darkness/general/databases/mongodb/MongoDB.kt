...
data class MongoDB(private val mongoData: MongoData) {
    ...
    constructor(config: Config) : this(
        try {
            ...
            MongoData.printDefaultMongoDataInConfigFile(
                config
            )
            println()
...
//    }
    ...
    fun connect(database: String): MongoClient {
        client =
            MongoClients.create(ConnectionString("mongodb://${mongoData.username}:${mongoData.password}@${mongoData.host}:${mongoData.port}/$database"))
        return client
    }
