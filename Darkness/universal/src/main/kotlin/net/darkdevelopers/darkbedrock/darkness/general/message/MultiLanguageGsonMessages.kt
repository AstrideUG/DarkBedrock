...
class MultiLanguageGsonMessages(config: GsonConfig) {
    ...
    val availableLanguages: MutableMap<String, GsonStringMapWithSubs> =
        mutableMapOf<String, GsonStringMapWithSubs>().apply {
            rawLanguages.forEach {
                this[it.key] =
                    GsonStringMapWithSubs(it.value.asJsonObject)
            }
        }
    /**
     * @author Lars Artmann | LartyHD
     * @since 15.10.2018
     */
    val availableLanguagesOnInit: Map<String, GsonStringMapWithSubs> =
        availableLanguages.toMap()
