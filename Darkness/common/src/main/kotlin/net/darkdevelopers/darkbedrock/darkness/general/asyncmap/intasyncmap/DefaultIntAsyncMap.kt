...
interface DefaultIntAsyncMap : IntAsyncMap {
    ...
    override fun removeIfEnough(uuid: UUID, key: String, count: Int, lambda: () -> Unit) =
        removeIfEnough(uuid, key, count, lambda) {}

    override fun removeIfEnough(uuid: UUID, key: String, count: Int, onSuccess: () -> Unit, onFail: () -> Unit) =
        hasEnough(uuid, key, count) { if (it) remove(uuid, key, count) { onSuccess() } else onFail() }
