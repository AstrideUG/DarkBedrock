...
//      } else humanEntity.sendMessage("${configService.prefix}${TEXT}Die Settings wurde schon ${IMPORTANT}gesetzt")
    ...
    private fun setItemToMapVoteInventory(base: IItemBuilder, slot: Int, name: String, size: Int) =
        mapVoteInventory.setItem(slot, base.setName("$SECONDARY$name").setLore(getLore(size)).setAmount(size).build())
