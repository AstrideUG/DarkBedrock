 ...
 */
package de.astride.apis.modules.base.plugin.util
...
object ModuleDependencyUtils {
	...
	fun sortCandidates(candidates: List<ModuleDescription>): List<ModuleDescription> {
		...
		val candidateMap = Maps
				.uniqueIndex(candidates) { d -> d?.id }
		// Add edges
	...
	private fun createLoopInformation(graph: Graph<ModuleDescription>): String {
		...
		for (edge in graph.edges()) {
			repr.append(edge.target().id).append(": [")
			repr.append(dependencyLoopInfo(graph, edge.target(), HashSet())).append("], ")
		}
	...
	private fun dependencyLoopInfo(graph: Graph<ModuleDescription>,
		...
		for (pd in graph.adjacentNodes(dependency)) {
			if (seen.add(pd)) {
				repr.append(pd.id).append(": [").append(dependencyLoopInfo(graph, dependency, seen))
						.append("], ")
			} else {
